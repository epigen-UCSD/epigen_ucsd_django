...
def getTotalReads(fq_dir):
    ...
    '''
    log_files=subprocess.check_output("find "+fq_dir+" -name DemultiplexingStats.xml",shell=True,universal_newlines=True).split()
    tree = ET.parse(log_files[0])
    flowcell = tree.getroot().getchildren()[0]
    cnts=[]
    for p in flowcell.findall('Project'):
        if p.get('name')=='all':
            for c in p.findall('./Sample/')[0].findall('./Lane/BarcodeCount'): cnts.append(int(c.text))
    return(sum(cnts))
...
def main(flowcell):
    ...
    obj.percent_of_reads_demultiplexed = round(total_lib_reads/obj.total_reads*100)
    obj.save()
