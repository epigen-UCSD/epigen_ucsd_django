...
def submit_singlecell(request):
    ...
    if request.method == 'GET':
        ...
        if not SeqInfo.objects.filter(seq_id=seq).exists():
            ...
            experiment_obj = ExperimentType.objects.get(
                experiment=experiment_type)
            print('refgenome submission: ', species,
                  experiment_type, experiment_obj)
            # get all refrence genomes available for this experiment type.

            refgenomes = RefGenomeInfo.objects.filter(
                species=species, experiment_type=experiment_obj).values('genome_name')
...
def submit_tenX(seq, refgenome, email):
    ...
    if experiment_type == '10xATAC':
        ...
        refgenome = RefGenomeInfo.objects.get(
            species=data['species'], genome_name=refgenome, experiment_type__experiment=experiment_type)
...
def insert_link(filename, seq, expt_type):
    ...
    if expt_type == '10xATAC':
        ...
        with open(filename, 'r') as f:
            f1 = f.readlines()  # f1 is list
        for num, line in enumerate(f1, start=1):
            if num == line_to_insert_at:
                newdata = newdata + '<div><h2><a class="data-link-epigen" style="margin-left:3em;" href="' + \
                    link+'"> Link to Output Data</a></h2><div>'
...
def view_websummary(request, seq_id):
    ...
    user = request.user
    seqinfo = SeqInfo.objects.select_related(
        'libraryinfo', 'libraryinfo__sampleinfo', 'libraryinfo__sampleinfo__group').get(seq_id=seq_id)
    permission = check_permissions(user=user, seqinfo=seqinfo)

    if permission == False:
        print('Access is denied to seq {seq_id} for user {user}'.format(
            seq_id=seq_id, user=user))
        raise PermissionDenied
    ...
    path = '{dir}/{seq_id}/outs/web_summary.html'.format(
        seq_id=seq_id, dir=dir)
    print('reading: ', path)
    file = open(path)
    data = file.read()
    if(LINK_CLASS_NAME not in data):
        ...
        data = file.read()
    if(data == None):
        print('ERROR: No data read in 10x Web_Summary.html File! for {seq_id}'.format(
            seq_id=seq_id))
